...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const result = Math.atan2(y2, x2) - Math.atan2(y1, x1);
...
function roundToPowerOfTen(num, pow) {
  const newNum = Array.from(Array(pow)).fill(0).join('');
  const stringify = String(num);
  const firstPart = stringify.slice(0, stringify.length - pow);
  const lastPart = stringify.slice(stringify.length - pow);
  const combined = `${firstPart}.${lastPart}`;
  const numbarized = Number(combined);
  const rounding = Math.round(numbarized + newNum);
  return Number(rounding + newNum);
}
